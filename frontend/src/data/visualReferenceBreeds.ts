...
// Source: https://datasets-server.huggingface.co/info?dataset=giacomov%2Ftsinghua_dogs
// The values below are the 130 ClassLabel names exposed by the public dataset metadata.
const visualReferenceClassLabels = [
...
function toDisplayName(label: string) {
  const technicalName = label.split("-", 3)[2] ?? label;
...
export const visualReferenceBreeds: VisualReferenceBreed[] = visualReferenceClassLabels.map((label) => ({
  id: label.split("-", 3)[1] ?? label,
  label,
  displayName: toDisplayName(label),
}));
